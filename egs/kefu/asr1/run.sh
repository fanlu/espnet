...
#train_dev=dev
#recog_set="test_kefu"
recog_set="kefu_eval_200h kefu_test3"
echo $stage
...
if [ ${stage} -le 1 ]; then
    ...
    #steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj 400 --write_utt2num_frames true \
    ...
    #    data/test_kefu exp/make_fbank/test_kefu ${fbankdir}
    ...
    #    ${feat_dt_dir}/storage
    #fi
    #dump.sh --cmd "$train_cmd" --nj 400 --do_delta $do_delta \
...
if [ ${stage} -le 3 ]; then
    ...
    mkdir -p ${lmdatadir}
    text2token.py -s 1 -n 1 -l ${nlsyms} data/train/text | cut -f 2- -d" " \
        > ${lmdatadir}/train.txt
    ...
    ${cuda_cmd} --gpu ${ngpu} ${lmexpdir}/train.log bash /mnt/cephfs2/asr/users/fanlu/espnet/tools/run_mgpu.sh ${ngpu} lm_train.py \
        --ngpu ${ngpu} \
        --backend ${backend} \
        --verbose 1 \
        --outdir ${lmexpdir} \
...
if [ ${stage} -le 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} -q ${queue} -l h="${host}" ${expdir}/train.log bash /mnt/cephfs2/asr/users/fanlu/run_mgpu.sh ${ngpu} asr_train.py \
        --ngpu ${ngpu} \
        --backend ${backend} \
        ...
        --etype ${etype} \
        --n_iter_processes ${n_iter_processes} \
        --elayers ${elayers} \
        --eunits ${eunits} \
        --eprojs ${eprojs} \
        --subsample ${subsample} \
...
if [ ${stage} -le 5 ]; then
	...
	echo ${feat_recog_dir}
        ...
        ${decode_cmd} JOB=1:${nj} ${expdir}/${decode_dir}/log/decode.JOB.log asr_recog.py \
            --ngpu ${ngpu} \
            --backend ${backend} \
